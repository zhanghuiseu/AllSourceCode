...
bool LFFSolverImpl::internalRunSolver(
  ...
  /*%%%%%%%%%%%%%%%%%%%  Begin add by zhanghui 20181026 %%%%%%%%%%%%%%%%%%%%%%%%*/
  //这个是为了区别不同的求解器，默认值是Z3，还可以是lff或者plff OR IIP
  if(SolverType::solverType == "IIP")
    SolverType::solverType = "IIP";
  else
    SolverType::solverType = "LFF";

  std::string Str;
  llvm::raw_string_ostream info(Str);
  ExprPPrinter::printQuery(info,query.constraints,query.expr);
  string kqueryInfo = info.str();
  clock_t start,end;
  ...
  calaSolver(SolverType::solverType,kqueryInfo,Z3TotalTime);
  /*%%%%%%%%%%%%%%%%%%%  End add by zhanghui 20181026 %%%%%%%%%%%%%%%%%%%%%%%%*/

...
//#############   begin: add by zhanghui20181025
   case LFF_SOLVER:
#ifdef ENABLE_Z3
    klee_message("Using LFF solver backend");
    return new LFFSolver();
#else
    klee_message("Not compiled with LFF Solver support");
    return NULL;
#endif

    case PLFF_SOLVER:
#ifdef ENABLE_Z3
    klee_message("Using PLFF solver backend");
    return new PLFFSolver();
#else
    klee_message("Not compiled with PLFF Solver support");
    return NULL;
#endif 
//#############   end: add by zhanghui20181025
//#############   begin: add by zhanghui20180306
   case IIP_SOLVER:
#ifdef ENABLE_Z3
    SolverType::solverType = "IIP";
    klee_message("Using IIP solver backend");
    return new LFFSolver();
#else
    klee_message("Not compiled with IIP Solver support");
    return NULL;
#endif
//#############   end: add by zhanghui20180306

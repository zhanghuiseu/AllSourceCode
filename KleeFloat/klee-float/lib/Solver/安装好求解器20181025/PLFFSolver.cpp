...
//这个参数是为了保证读取配置文件LFFSolver.config只一次
static bool config = true;
//这个是求解器的选择，-1表示选择Z3,0表示选择LFFSolver，1表示选择PLFFSolver
static int uselff = -1;
...
namespace {
llvm::cl::opt<std::string> PLFFQueryDumpFile(
    "PLFF-query-dump", llvm::cl::init(""),
    llvm::cl::desc("Dump PLFF's representation of the query to the specified path"));

llvm::cl::opt<bool> PLFFValidateModels(
    "PLFF-validate-models", llvm::cl::init(false),
    llvm::cl::desc("When generating PLFF models validate these against the query"));

llvm::cl::opt<bool> PLFFAckermannizeArrays(
    "PLFF-array-ackermannize", llvm::cl::init(true),
    llvm::cl::desc("Try to ackermannize arrays before building PLFF queries "
...
class PLFFSolverImpl : public SolverImpl {
private:
  Z3Builder *builder;
  double timeout;
...
PLFFSolver::PLFFSolver() : Solver(new PLFFSolverImpl()) {}

char *PLFFSolver::getConstraintLog(const Query &query) {
  return impl->getConstraintLog(query);
}

void PLFFSolver::setCoreSolverTimeout(double timeout) {
  impl->setCoreSolverTimeout(timeout);
}

char *PLFFSolverImpl::getConstraintLog(const Query &query) {
  ...
  ::Z3_string result = Z3_benchmark_to_smtlib_string(
      builder->ctx,
      /*name=*/"Emited by klee::PLFFSolverImpl::getConstraintLog()",
      /*logic=*/"",
      /*status=*/"unknown",
      /*attributes=*/"",
...
bool PLFFSolverImpl::computeTruth(const Query &query, bool &isValid) {
  bool hasSolution;
  bool status =
...
bool PLFFSolverImpl::computeValue(const Query &query, ref<Expr> &result) {
...
bool PLFFSolverImpl::computeInitialValues(
    ...
    std::vector<std::vector<unsigned char> > &values, bool &hasSolution) 
{
   ...
   //Modfic By Zhanghui 20180511
   //read config file
  if(config == true)
  {
    ifstream read;  
    string filePath = "/home/zy/KleeFloat/KLEE-FLOAT/klee-float/lib/Solver/LFFSolver.config";
    ...
    if(read.is_open()==true)
    {
      ...
      while(getline(read,one))
        allConfig.push_back(one);
      if(allConfig.size()>0)
      {
        string choose = allConfig[0].substr(allConfig[0].find(':')+1);
        //这个是求解器的选择，-1表示选择Z3,0表示选择LFFSolver,1表示选择PLFFSolver
        if(choose == "LFFSolver")
          uselff = 0;
        else if(choose == "PLFFSolver")
          uselff = 1;
        else
          uselff = -1;
      }
  ...
  ExprPPrinter::printQuery(info,query.constraints,query.expr);
  std::cout<<"KQuery is As Following: \n "<<info.str()<<std::endl<<std::endl;

  //-1表示选择Z3
  if(uselff == -1)
  {
    ofstream fd;  
    string filePath = "/home/zy/KleeFloat/KLEE-FLOAT/klee-float/examples/AllResult/Z3SolverRes/" + getFileName(pathIndex);
    pathIndex += 1;
    fd.open(filePath,ios::out);
   
    clock_t start,end;
    bool Z3Res = false;
    double Z3TotalTime=0.0;
    start = clock();
    Z3Res = internalRunSolver(query, &objects, &values, hasSolution);
    end = clock();
    Z3TotalTime = (double)(end-start)/CLOCKS_PER_SEC;

    string Z3VarName = "" , Z3VarType = "" , Z3VarData = "";
    for(int index1 = 0; index1 < (int)objects.size(); index1++)
    {
      //cout<<array->size<<"   "<<array->name<<endl;
      const Array *array = objects[index1];
      vector<unsigned char> data = values[index1];
      Z3VarName = Z3VarName + array->name + ",";
      string ttmp= getZ3VarType(array->name);
      Z3VarType = Z3VarType + ttmp + ",";
      Z3VarData = Z3VarData + getZ3VarData(ttmp,data) + ",";
    }
    if(Z3VarName.back()==',')
      Z3VarName.pop_back();
    if(Z3VarType.back()==',')
    ...
    fd<<"KQuery Path : "<<info.str()<<"\n";
    fd<<"Z3VarName : "<<Z3VarName<<"\n";
    fd<<"Z3VarType : "<<Z3VarType<<"\n";
    fd<<"Z3VarData : "<<Z3VarData<<"\n";
    fd<<"Z3Res : "<<Z3Res<<"\n";
    fd<<"Z3TotalTime : "<<Z3TotalTime<<"\n";
    fd.close();
  ...
  }else if(uselff == 0) 
  {
      //Modife By jackzhang 
      clock_t start,end;
      bool Z3Res = false, LFFRes = false;
      double Z3TotalTime=0.0,LFFTotalTime=0.0;
      start = clock();
      Z3Res = internalRunSolver(query, &objects, &values, hasSolution);
      end = clock();
      Z3TotalTime = (double)(end-start)/CLOCKS_PER_SEC;
      ...
      std::string solverpath="/home/zy/KleeFloat/LFFSolver/LFFSolver.jar";
      char* type = NULL;
      char* name = NULL;
      ...
      start = clock();
      LFFRes = beginParserAndSolver(info.str().c_str(),solverpath.c_str(),type,name,constraint,isValid,resType,coverage,solverRes);
      end = clock();
      LFFTotalTime = (double)(end-start)/CLOCKS_PER_SEC;
      sleep(2);

      fd<<"KQuery Path : "<<info.str()<<"\n";
      fd<<"SolverPath : "<<solverpath<<"\n";
      fd<<"Type : "<<type<<"\n";
      fd<<"Name : "<<name<<"\n";
      fd<<"Constraint : "<<constraint<<"\n";
      fd<<"ResType : "<<resType<<"\n";
      fd<<"Coverage : "<<coverage<<"\n";
      fd<<"SolverRes : "<<solverRes<<"\n";
      fd<<"LFFTotalTime : "<<LFFTotalTime<<"\n";
      fd<<"Z3Res : "<<Z3Res<<"\n";
      fd<<"Z3TotalTime : "<<Z3TotalTime<<"\n";
      fd.close();
      ...
      cout<<"Z3     Res: "<<Z3Res<<"     , Time: "<<Z3TotalTime<<endl;
      cout<<"LFFRes Res: "<<LFFRes<<"    , Time: "<<LFFTotalTime<<endl;
      std::cout<<"*********************parserAndSolver finished***************************"<<std::endl<<std::endl;
      return Z3Res;
  ...
  }else if(uselff == 1)
  {
      //Modife By jackzhang 
      clock_t start,end;
      bool Z3Res = false, PLFFRes = false;
      double Z3TotalTime=0.0,PLFFTotalTime=0.0;
      start = clock();
      Z3Res = internalRunSolver(query, &objects, &values, hasSolution);
      end = clock();
      Z3TotalTime = (double)(end-start)/CLOCKS_PER_SEC;
      ...
      std::string solverpath = "/home/zy/cudaWorkspaceServer/JavaJar/AGPC.jar";
      char* type = NULL;
      char* name = NULL;
      char* constraint = NULL;
      bool isValid;
      ...
      start = clock();
      PLFFRes = beginParserAndPLFFSolver(info.str().c_str(),solverpath.c_str(),type,name,constraint,isValid,resType,coverage,solverRes);
      end = clock();
      ...
      fd<<"Coverage : "<<coverage<<"\n";
      fd<<"SolverRes : "<<solverRes<<"\n";
      fd<<"PLFFTotalTime : "<<PLFFTotalTime<<"\n";
      fd<<"Z3Res : "<<Z3Res<<"\n";
      fd<<"Z3TotalTime : "<<Z3TotalTime<<"\n";
      fd.close();
      ...
      cout<<"Z3     Res: "<<Z3Res<<"     , Time: "<<Z3TotalTime<<endl;
      cout<<"PLFFRes Res: "<<PLFFRes<<"    , Time: "<<PLFFTotalTime<<endl;
      std::cout<<"*********************parserAndSolver finished***************************"<<std::endl<<std::endl;
      return Z3Res;
  }
...
bool PLFFSolverImpl::internalRunSolver(
    const Query &query, const std::vector<const Array *> *objects,
    std::vector<std::vector<unsigned char> > *values, bool &hasSolution) {
...
SolverImpl::SolverRunStatus PLFFSolverImpl::handleSolverResponse(
    ::Z3_solver theSolver, ::Z3_lbool satisfiable,
    const std::vector<const Array *> *objects,
    std::vector<std::vector<unsigned char> > *values, bool &hasSolution,
    FindArrayAckermannizationVisitor &ffv,
...
bool PLFFSolverImpl::validateZ3Model(::Z3_solver &theSolver, ::Z3_model &theModel) {
  bool success = true;
...
void PLFFSolverImpl::ackermannizeArrays(
...
SolverImpl::SolverRunStatus PLFFSolverImpl::getOperationStatusCode() {

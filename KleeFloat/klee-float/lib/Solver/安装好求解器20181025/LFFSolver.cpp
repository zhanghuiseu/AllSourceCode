...
namespace {
llvm::cl::opt<std::string> LFFQueryDumpFile(
    "LFF-query-dump", llvm::cl::init(""),
    llvm::cl::desc("Dump LFF's representation of the query to the specified path"));

llvm::cl::opt<bool> LFFValidateModels(
    "LFF-validate-models", llvm::cl::init(false),
    llvm::cl::desc("When generating LFF models validate these against the query"));

llvm::cl::opt<bool> LFFAckermannizeArrays(
    "LFF-array-ackermannize", llvm::cl::init(true),
    llvm::cl::desc("Try to ackermannize arrays before building LFF queries "
...
class LFFSolverImpl : public SolverImpl {
private:
  Z3Builder *builder;
  double timeout;
...
LFFSolver::LFFSolver() : Solver(new LFFSolverImpl()) {}

char *LFFSolver::getConstraintLog(const Query &query) {
  return impl->getConstraintLog(query);
}

void LFFSolver::setCoreSolverTimeout(double timeout) {
  impl->setCoreSolverTimeout(timeout);
}

char *LFFSolverImpl::getConstraintLog(const Query &query) {
  ...
  ::Z3_string result = Z3_benchmark_to_smtlib_string(
      builder->ctx,
      /*name=*/"Emited by klee::LFFSolverImpl::getConstraintLog()",
      /*logic=*/"",
      /*status=*/"unknown",
      /*attributes=*/"",
...
bool LFFSolverImpl::computeTruth(const Query &query, bool &isValid) {
  bool hasSolution;
  bool status =
...
bool LFFSolverImpl::computeValue(const Query &query, ref<Expr> &result) {
...
bool LFFSolverImpl::computeInitialValues(
...
bool LFFSolverImpl::internalRunSolver(
    const Query &query, const std::vector<const Array *> *objects,
    std::vector<std::vector<unsigned char> > *values, bool &hasSolution) {
...
SolverImpl::SolverRunStatus LFFSolverImpl::handleSolverResponse(
    ::Z3_solver theSolver, ::Z3_lbool satisfiable,
    const std::vector<const Array *> *objects,
    std::vector<std::vector<unsigned char> > *values, bool &hasSolution,
    FindArrayAckermannizationVisitor &ffv,
...
bool LFFSolverImpl::validateZ3Model(::Z3_solver &theSolver, ::Z3_model &theModel) {
  bool success = true;
...
void LFFSolverImpl::ackermannizeArrays(
...
SolverImpl::SolverRunStatus LFFSolverImpl::getOperationStatusCode() {

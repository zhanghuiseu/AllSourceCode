	 ...
	 * LFF Solver 的总的开始部分
	 * */
	void runLFFSolver::run(string& res)
	{
		//cudaProfilerStart();
		...
		for(int indexOfRun = 1 ; indexOfRun <= SolverParameter::countOfRepeation; indexOfRun++)
		{
			...
			{
				if(isCovered)
					res = "1@"+ MathFunction::toString(SolverParameter::finalCovered)+","+MathFunction::toString(totalTime)+"@";
				else
					res = "0@"+ MathFunction::toString(SolverParameter::finalCovered)+","+MathFunction::toString(totalTime)+"@";
				if(ConstraintParameter::NUM_OF_PARAM<=0)
					res += "1";
				else
				{
					res += MathFunction::toString(SolverParameter::finalParams[0]);
					for(int i = 1; i < ConstraintParameter::NUM_OF_PARAM; i++)
						res += ","+MathFunction::toString(SolverParameter::finalParams[i]);;
				}
			}else
			{
				res = "2@0,0@"+ ConstraintParameter::noSolutionInfo;
			}
